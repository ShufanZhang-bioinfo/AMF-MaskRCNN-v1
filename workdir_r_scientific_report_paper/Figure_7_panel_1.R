...
col.overall.ms <- col.overall %>%
  ...
  dplyr::mutate(cube_root_percent_colinization = percent_colonization^(1/3))
met <- "Percent Colonization"
p <- ggplot2::ggplot(data = col.overall.ms, aes(x = region, y = cube_root_percent_colinization, fill = region)) +
  geom_violin() +
...
data.table::fwrite(file = "results/Figure_7_panel_1.txt", x= dat)
ggsave(filename = "results/Figure_7_panel_1.pdf", plot = p, device = "pdf", width = 6, height = 6, units = 'in')
