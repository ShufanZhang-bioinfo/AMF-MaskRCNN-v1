...
col.class.count2.sub.ms <- col.class.count2.sub %>%
...
p <- ggplot2::ggplot(col.class.count2.sub.ms, aes(x=factor(accession, levels = accession_order_pc),y=count_density^0.5,fill=class2)) + 
...
data.table::fwrite(file = "results/Figure_8_panel_2.txt", x= dat)
ggsave(filename = "results/Figure_8_panel_2.pdf", plot = p, device = "pdf", width = 8, height = 6, units = 'in')
