...
col.overall.ms <- col.overall %>%
  ...
  dplyr::mutate(square_root_count_density = count_density^.5)

my_comparisons <- list( c("Top", "Middle"), c("Middle", "Bottom"), c("Top", "Bottom") )

p <- ggplot2::ggplot(data = col.overall.ms, aes(x = region, y = square_root_count_density, fill = region)) +
  geom_violin() +
  geom_boxplot(fill = "white", width = 0.1) +
  geom_jitter(size = 0.5, width = 0.2) +
  scale_fill_manual(values = colors) +
  ylab(expression("Count Density"^(1/2))) +
  xlab("") +
  labs(fill = "Region") +
  theme_bw()

dat <- as.data.frame(p$data)
data.table::fwrite(file = "results/Figure_7_panel_2.txt", x= dat)
ggsave(filename = "results/Figure_7_panel_2.pdf", plot = p, device = "pdf", width = 6, height = 6, units = 'in')
