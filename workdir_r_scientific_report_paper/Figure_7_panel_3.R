...
seg.amf.sum.class.ms <- seg.amf.sum.class %>%
...
p <- ggplot(seg.amf.sum.class.ms, aes(x=region, y=log(avg.size), fill = region)) +
  geom_boxplot() +
  geom_jitter(size = 0.2, width =0.2)+
  labs(fill = "Region") +
  xlab("") +
  ylab("Log (Average Size)") +
...
data.table::fwrite(file = "results/Figure_7_panel_3.txt", x= dat)
ggsave(filename = "results/Figure_7_panel_3.pdf", plot = p, device = "pdf", width = 8, height = 6, units = 'in')
