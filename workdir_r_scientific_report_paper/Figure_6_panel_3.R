...
p <- ggplot(var.pc, aes(x = x.arbc, y = var.accession))+
  geom_smooth(stat = "smooth") +
  xlab("Arbuscule count") +
  ylab(expression("Accession variance")) +
  labs(color = "Acccession") +
  theme_bw()
  
dat <- as.data.frame(p$data)
data.table::fwrite(file = "results/Figure_6_panel_3.txt", x= dat)
ggsave(filename = "results/Figure_6_panel_3.pdf", plot = p, device = "pdf", width = 6, height = 6, units = 'in')
